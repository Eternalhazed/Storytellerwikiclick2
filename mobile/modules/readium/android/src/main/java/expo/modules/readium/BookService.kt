...
class BookService(private val context: Context) {
            ...
            Streamer(
                    ...
                    onCreatePublication = { ->
                        ...
                        runBlocking {
                            ...
                            val packageDocument =
                                    PackageDocument.parse(opfXmlDocument, opfPath)
                                            ?: return@runBlocking

                            val manifestItems = packageDocument.manifest
                            @Suppress("UNCHECKED_CAST")
                            val itemById =
                                    manifestItems.filter { it.id != null }.associateBy(Item::id) as
                                            Map<String, Item>
                            ...
                            for (manifestItem in manifestItems) {
                                ...
                                val linkIndex =
                                        readingOrder.indexOfFirstWithHref(manifestItem.href)
                                                ?: continue
                                val link = readingOrder[linkIndex]
                                readingOrder[linkIndex] =
                                        link.addProperties(
                                                mapOf("mediaOverlay" to mediaOverlayHref)
                                        )
                            }

                            builder.manifest = builder.manifest.copy(readingOrder = readingOrder)
                        }
                    }
            )
    ...
    fun getClip(bookId: Long, locator: Locator): Clip? {
        ...
        val link =
                try {
                    ...
                    throw Exception(
                            "Locator ${locator.href} could not be found in reading order for book $bookId"
                    )
                }
        val overlayHref =
                try {
    ...
    private suspend fun locateFromPositions(bookId: Long, link: Link): Locator {
        ...
        val readingOrderIndex =
                publication.readingOrder.indexOfFirstWithHref(link.href)
                        ?: throw Exception(
                                "Could not find a locator for href ${link.href} in reading order for book $bookId"
                        )
    ...
    suspend fun buildFragmentLocator(bookId: Long, href: String, fragment: String): Locator {
        ...
        val defaultLocator = Locator(href = href, type = "application/xhtml+xml")
        ...
        val startOfChapterProgression =
                locateFromPositions(bookId, link).locations.totalProgression
                        ?: return defaultLocator
        val chapterIndex =
                publication.readingOrder.indexOfFirstWithHref(link.href) ?: return defaultLocator
        ...
        val startOfNextChapterProgression =
                nextChapterIndex.let {
                    if (it == publication.readingOrder.size) {
                        ...
                        return@let locateFromPositions(bookId, nextChapterLink)
                                .locations
                                .totalProgression
                    }
                }
